...
export const applyJob = asyncHandler(async (req, res) => {
  try {
    ...
    const user = await User.findOne({ auth0Id: req.auth.sub });
    if (!user) return res.status(404).json({ message: "User not found" });
    if (job.applicants.includes(user._id)) return res.status(400).json({ message: "Already applied for this job" });
...
export const likeJob = asyncHandler(async (req, res) => {
  try {
    ...
    if (isLiked) { job.likes = job.likes.filter((like) => !like.equals(user._id)); } else { job.likes.push(user._id); }
    await job.save();
    return res.status(200).json(job);
  } catch (error) {
